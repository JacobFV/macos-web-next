<script lang="ts">
	...
	// Network isolation: the Slack app only ever talks to channels namespaced
	// with the `slack:` prefix, so it can never surface iMessage/Teams traffic.
	const NETWORK = 'slack';
	...
	async function refresh_channels() {
		...
		if (r && Array.isArray(r.channels)) {
			// Only keep channels on the slack network; display them prefix-stripped.
...
<style>
	...
	.titlebar {
		...
		min-height: 36px;
		background: #4a154b;
		border-bottom: 1px solid #3a1139;
		font-size: 12px;
		color: #e8d9e8;
	}
	.status-dot {
	...
	.me { font-weight: 600; color: #cdb4cd; }
	.space { flex: 1; }
	...
	.conversation.active { background: #1164a3; color: #ffffff; }
	...
	.convo-name { font-size: 14px; font-weight: 500; color: #d9c9da; }
	.convo-count { font-size: 11px; opacity: 0.6; color: #d9c9da; }
	.convo-preview {
		font-size: 12px;
		opacity: 0.6;
		color: #d9c9da;
	...
	.chat-name { font-size: 16px; font-weight: 800; color: #1d1c1d; }
	...
	.message.sent { background: #f4f0f4; }
	.msg-line { display: flex; align-items: baseline; gap: 8px; }
	.sender { font-size: 13px; font-weight: 700; color: #1d1c1d; }
	.timestamp { font-size: 11px; color: #999; }
	.text { font-size: 14px; line-height: 1.46; color: #1d1c1d; }
	...
	.composer:focus { border-color: #611f69; box-shadow: 0 0 0 1px #611f69; }
	.send-btn {
		padding: 0 18px;
		min-width: 70px;
		border-radius: 8px;
